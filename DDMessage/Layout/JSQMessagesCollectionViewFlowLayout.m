...
- (CGSize)messageBubbleSizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([messageItem isMediaMessage]) {
      ...
      if(![messageItem isSystemMessage]){
        maximumTextWidth = self.itemWidth - avatarSize.width - self.messageBubbleLeftRightMargin - horizontalInsetsTotal;
      }else{
        maximumTextWidth =200;
      }
...
- (CGFloat)sizeForHorizontalWidthForMessageContainerAtIndexPath:(NSIndexPath *)indexPath
{
  id<JSQMessageData> messageItem = [self.collectionView.dataSource collectionView:self.collectionView messageDataForItemAtIndexPath:indexPath];
  ...
  if([messageItem isSystemMessage]){
    ...
    //  from the cell xibs, there is a 2 point space between avatar and bubble
    CGFloat width = (320 - messageBubbleSize.width)/2-30;
    if (width < 0){
      return 0;
    }else
      return width;
...
- (CGSize)sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  if([messageItem isSystemMessage]){
    ...
    CGFloat maximumTextWidth;
    maximumTextWidth = self.itemWidth - 2*avatarSize.width - self.messageBubbleLeftRightMargin - horizontalInsetsTotal;
    messageBubbleSize.width = maximumTextWidth;
  }
...
- (void)jsq_configureMessageCellLayoutAttributes:(JSQMessagesCollectionViewLayoutAttributes *)layoutAttributes
{
  ...
  layoutAttributes.messageContainerHorizontalWidth = [self.collectionView.delegate collectionView:self.collectionView layout:self HorizontalForMessageContainerViewAtIndexPath:indexPath];
}
